...
// بعلامة "✦" ذهبية مضمَّنة في النص نفسه (بدل رسمها كعنصر منفصل كما كان سابقاً مع ◆، لتفادي
// ازدواج العلامة). هذه الدالة تفصل الرمز عن بقية الجملة لتلوين الرمز وحده بالذهبي المعدني،
// مع إبقاء النص نفسه دون أي تغيير. إن لم يبدأ النص بـ"✦ " لأي سبب (نص لم يُحدَّث بعد) تُعرض
// الجملة كاملة كما هي بدل كسر الواجهة.
function renderBulletFeature(text) {
  if (typeof text === "string" && text.startsWith("✦ ")) {
    return (
      <>
        <span aria-hidden="true" style={{ color: AGENCY_COLORS.metallicGold, marginInlineEnd: "0.45rem" }}>
          ✦
        </span>
...
// نافذة "Sign In" الفاخرة (LoginModal) — سقالة واجهة (UI scaffold) لمنع إساءة استخدام الباقة
// المجانية عبر إجبار تسجيل الدخول قبل التوليد. ⚠️ ملاحظة مهمة جداً: هذا حالياً مُموَّه (mock)
// بالكامل بناءً على طلب صريح ("mock the auth state... before integrating Firebase/Clerk") —
// زرا "Continue with Google" و"Continue with Email" لا يستدعيان Firebase Auth الحقيقي إطلاقاً
// (لا signInWithPopup ولا إرسال بريد OTP فعلي)، بل يكتفيان بضبط isAuthenticated=true محلياً في
// AppContent عبر onMockSuccess. هذا يعني عملياً صفر حماية حقيقية ضد إساءة الاستخدام إلى أن
// يُستبدل هذا المنطق باستدعاء حقيقي لـ Firebase (auth.js يحتوي بالفعل GoogleAuthProvider جاهزاً
// للاستخدام) أو مزوّد OTP فعلي. تصميم النافذة نفسها بنفس لغة "الزجاج الفاخر" (Glassmorphism)
// المستخدمة في شريط "Executive Teaser Banner" أدناه: خلفية شبه شفافة + backdrop-filter blur +
// حدّ ذهبي خفيف، فوق طبقة تعتيم داكنة تغطي الصفحة بالكامل.
function LoginModal({ open, t, onClose, onMockSuccess }) {
  // "choice": الشاشة الأولى (Google / Email). "email": إدخال البريد. "otp": إدخال رمز التحقق
  // المكوّن من 6 أرقام. لا وجود لأي استدعاء شبكة حقيقي في أي من هذه المراحل — كلها محلية بحتة.
  const [step, setStep] = useState("choice");
  const [email, setEmail] = useState("");
  const [otp, setOtp] = useState("");
  const [errorKey, setErrorKey] = useState("");

  // يعيد ضبط كل شيء في كل مرة تُفتح فيها النافذة من جديد.
  useEffect(() => {
    if (open) {
      setStep("choice");
      setEmail("");
  ...
  // Google (Mock): في نظام حقيقي هذا سيستدعي signInWithPopup(auth, googleProvider) من
  // firebase.js (المزوّد جاهز بالفعل هناك، googleProvider، ويُستخدم فعلياً في Auth.js لتسجيل
  // الدخول الحقيقي) — هنا فقط ننجح فوراً محلياً لأغراض اختبار الواجهة.
  ...
  const handleSendCode = () => {
    ...
    setErrorKey("");
    // Mock: لا يُرسَل أي بريد إلكتروني فعلي هنا — مجرد انتقال للخطوة التالية من الواجهة.
    setStep("otp");
  };

  const handleVerifyOtp = () => {
    if (otp.trim().length !== 6 || !/^\d{6}$/.test(otp.trim())) {
      setErrorKey("otpInvalid");
      return;
    }
    setErrorKey("");
    // Mock: أي رمز مكوَّن من 6 أرقام يُقبَل هنا — لا تحقق حقيقي مقابل رمز أُرسِل فعلياً.
    onMockSuccess();
  };
    ...
    >
      <div
        ...
        style={{
          ...
          width: "100%",
          textAlign: "center",
          boxShadow: "0 20px 40px -12px rgba(10, 25, 47, 0.5)",
        }}
      >
        <h2 style={{ margin: "0 0 0.6rem", color: "white", fontWeight: "800", letterSpacing: "-0.01em" }}>
          {t("loginModal.headline")}
        </h2>
        <p style={{ color: "rgba(248, 250, 252, 0.75)", lineHeight: 1.6, marginBottom: "1.75rem", fontSize: "0.92rem" }}>
        ...
        {step === "choice" && (
          <>
            <button
              ...
              className="agency-btn-primary"
              style={{ width: "100%", padding: "0.9rem", borderRadius: "8px", fontSize: "0.95rem", marginBottom: "0.75rem" }}
            >
              {t("loginModal.googleBtn")}
            </button>
            <button
        ...
        {step === "email" && (
          <>
            <label style={{ display: "block", textAlign: "start", color: "rgba(248,250,252,0.85)", fontSize: "0.85rem", fontWeight: "600", marginBottom: "0.4rem" }}>
            ...
            />
            {errorKey && (
              <p style={{ color: "#fca5a5", fontSize: "0.85rem", marginBottom: "1rem", textAlign: "start" }}>
                {t(`loginModal.${errorKey}`)}
              </p>
            )}
            <button
              onClick={handleSendCode}
              className="agency-btn-primary"
              style={{ width: "100%", padding: "0.9rem", borderRadius: "8px", fontSize: "0.95rem", marginBottom: "0.6rem" }}
            >
              {t("loginModal.sendCodeBtn")}
            </button>
            <button
              onClick={() => {
                setStep("choice");
              ...
              style={{ background: "transparent", border: "none", color: "rgba(248,250,252,0.7)", fontSize: "0.85rem", cursor: "pointer", padding: "0.4rem" }}
            >
              {t("loginModal.backBtn")}
            </button>
          </>
        )}
        ...
        {step === "otp" && (
          <>
            <label style={{ display: "block", textAlign: "start", color: "rgba(248,250,252,0.85)", fontSize: "0.85rem", fontWeight: "600", marginBottom: "0.4rem" }}>
              {t("loginModal.otpLabel")}
            </label>
            <p style={{ color: "rgba(248,250,252,0.65)", fontSize: "0.82rem", textAlign: "start", marginBottom: "0.6rem" }}>
              {t("loginModal.otpHint", { email })}
            </p>
            <input
              type="text"
              inputMode="numeric"
...
// نافذة "Agency Ambassadors" — عنصر نائب (placeholder) بسيط لرابط "Elite Partner Program"
...
function AppContent() {
  ...
  // من Firebase (onAuthStateChanged أدناه) لأن هذا بالضبط ما طُلب: حالة منفصلة يُتحكَّم بها
  // يدوياً من أزرار تسجيل الدخول الوهمية، تمهيداً لدمج Firebase/Clerk حقيقي لاحقاً في هذا
  // المكان بالذات. لتفادي أي تراجع (regression) في السلوك الحالي: AppContent بأكملها محمية
  // أصلاً خلف بوابة Firebase حقيقية (انظر `if (!user) return <Auth />` أسفل الملف) — أي
  // مستخدم يصل فعلياً لهذا الكود يكون مسجَّلاً دخوله بالفعل بحساب Firebase حقيقي. لذلك تتزامن
  // isAuthenticated تلقائياً مع `user` الحقيقي عبر useEffect أدناه، فلا يرى أي مستخدم حالي
  // نافذة تسجيل الدخول الوهمية هذه إطلاقاً حتى تُفعَّل لاحقاً (مثلاً بتخفيف بوابة Firebase
  // للسماح بتصفح عام قبل تسجيل الدخول). الزر "Commission Campaign Strategy" يتحقق من
  // isAuthenticated أولاً (راجع handleGenerate)، تماماً كما طُلب.
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [showLoginModal, setShowLoginModal] = useState(false);

  // حالة نافذة "Agency Ambassadors" — عنصر نائب (placeholder) لرابط "Elite Partner Program"
  // في تذييل الصفحة (راجع AmbassadorModal أعلاه للتفاصيل).
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      // مزامنة سقالة المصادقة الوهمية مع حالة Firebase الحقيقية — راجع التعليق أعلاه.
  ...
  // ينجَح عند الضغط على أي من زرَّي تسجيل الدخول الوهميَّين (Google أو Email OTP) داخل
  // LoginModal — Mock بحت، لا يُنشئ أي جلسة Firebase حقيقية (راجع التحذير أعلى LoginModal).
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: AGENCY_COLORS.pearl, padding: "2rem", fontFamily: "system-ui" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          {/* التصميم الطباعي (Typography): "agency-h1" في App.css يضبط الوزن/الحجم المتجاوب/تباعد
          ...
          {tier === "free" && (
            ...
            >
              ...
              <p
                style={{
                  margin: 0,
                  // "#F1F5F9" (يعادل text-slate-100) بدل rgba شبه شفاف سابقاً + fontWeight 500
                  // (font-medium) بدل الوزن العادي — لضمان وضوح كامل فوق خلفية الزجاج الداكنة
                  // (Glassmorphism)، ضمن معالجة "Readability Overhaul".
                  color: "#F1F5F9",
                  fontWeight: "500",
                  fontSize: "0.92rem",
                  lineHeight: 1.75,
          ...
          <div className="agency-card" style={{ borderRadius: "16px", padding: "2rem", marginBottom: "2rem" }}>
            ...
            {/* "Luxury Structured Tier Cards" — تُعرض ميزات الباقة المدفوعة كبنود مستقلة سهلة
                ...
                سبرنتات أسبوعية بحد أقصى 7 أيام لكل سبرنت، بدل مخطط متصل 30/90 يوماً سابقاً).
                نصوص البنود جاهزة بالكامل من planFeatures.*.features في i18n.js (تتضمّن "✦"
                داخل النص نفسه)، مطابقة تماماً لحدود PLAN_MAX_DAYS/PLAN_MAX_GENERATIONS أعلاه
                ...
                {["conversionArchitecture", "competitorGap", "omnichannelGrowth"].map((cardKey) => (
                  ...
                  >
                    ...
                    {/* طبقة تعتيم زجاجية (frosted glass overlay) — نصف شفافة + backdrop-filter
                        blur فوق المحتوى المموّه أعلاه، لإحساس "زجاج فاخر" متناسق مع شريط
                        الإفصاح أعلى الصفحة. */}
                    <div
                      style={{
                        position: "absolute",
                    ...
                    {/* مسح ذهبي لامع (shimmer sweep) — طبقة زخرفية إضافية فوق كل شيء، غير مرئية
                        إلا عند :hover/:focus-visible (راجع .locked-feature-card في App.css)؛
                        aria-hidden لأنها بصرية بحتة ولا تحمل أي معنى دلالي. */}
        ...
        {/* تذييل فاخر (Luxury Footer) — زر "Elite Partner Program" (TASK 2: أُعيد تصميمه من رابط
            نصي باهت صغير إلى كبسولة (pill) فاخرة متوهجة بالذهبي/العنبري عبر className
            "agency-affiliate-btn" المعرَّف في App.css، والذي يطبّق حرفياً — كـ CSS عادي بدل
            Tailwind — فئات Tailwind الدقيقة المطلوبة (mt-8 mb-4 mx-auto inline-flex
            items-center justify-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r
            from-amber-500/10 to-orange-500/10 border border-amber-500/40 text-amber-500
            font-bold text-base hover:bg-amber-500/20 hover:-translate-y-1 transition-all
            duration-300 cursor-pointer shadow-[0_0_15px_rgba(245,158,11,0.2)]
            backdrop-blur-md). ⚠️ ملاحظة مهمة: هذا المشروع لا يملك تكامل Tailwind فعلياً مفعَّلاً
            في سلسلة بناء Create React App الحالية (react-scripts عادي، بدون craco أو
            react-app-rewired أو postcss.config.js يربط tailwindcss/postcss/autoprefixer
            الموجودة في package.json فعلياً بعملية البناء) — لو استُخدمت أصناف Tailwind هنا
            حرفياً كـ className، كانت ستُطبَع كنص خام بلا أي تأثير بصري إطلاقاً على جهاز
            المستخدم. راجع الرد النهائي للمطوّر لشرح كامل وخيار ربط Tailwind فعلياً لاحقاً إن
            رغب المستخدم بذلك. يفتح الزر نافذة "Agency Ambassadors" (عنصر نائب/placeholder
            بسيط، راجع AmbassadorModal أعلاه؛ برنامج الشركاء عبر Lemon Squeezy لم يُطلَق بعد). */}
        <div
          style={{
            maxWidth: "1100px",
            margin: "3rem auto 0",
        ...
        >
          <button onClick={() => setShowAmbassadorModal(true)} className="agency-affiliate-btn">
      ...
      <LoginModal
        ...
        onMockSuccess={handleMockLoginSuccess}
      />
